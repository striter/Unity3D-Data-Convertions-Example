...
namespace TExcel
{
    ...
    class Properties<T> where T : struct,ISExcel
    {
        ...
        {
            m_Properties =  Tools.GetFieldData<T>(Tools.ReadExcelFirstSheetData(Resources.Load<TextAsset>(typeof(T).Name)));
        }
    ...
    class SheetProperties<T> where T : struct, ISExcel
    {
        ...
        public static void Init()
        {
            ...
            Dictionary<string, List<string[]>> m_AllDatas = Tools.ReadExcelMultipleSheetData(Resources.Load<TextAsset>(typeof(T).Name));
